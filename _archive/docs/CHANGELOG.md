...
2026-07-03: Add: yukleme gorunurluk ayarlari — --publish-at "YYYY-MM-DD HH:MM" ile tam tarih-saat programli yayin (private->publishAt); dil defaultLanguage/AudioLanguage=en (--lang); kategori Sports(17); otomatik 3-5 alakali etiket (_build_tags). madeForKids false. (upload.py, youtube.py)
2026-07-01: Fix: altyazı konumu — ASS'e PlayResX/Y=1080x1920 eklendi (yoktu → libass 384x288 varsayıyor, piksel değerleri ekran dışına kaçıyordu/altyazı kayboluyordu). format1: fontsize 38→78, margin_bottom 160→420 (alt-üçte-bir, Shorts UI'nin üstünde). Artık pikseller gerçek ekrana denk (captions.py, format1.json)
